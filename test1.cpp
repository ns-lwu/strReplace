...
int main(){
    ...
    // std::ifstream longif("longstr.txt");
    // std::stringstream buffer;
    // buffer << longif.rdbuf();
    // std::string longStr = buffer.str();
//     std::string longStr = "“Heavens! what a virulent attack!” replied the prince, not in theleast dis\
// concerted by this reception. He had just entered, wearing anembroidered c\
// ourt uniform, knee breeches, and shoes, and had stars onhis breast and a serene expre\
// ssion on his flat face. He spoke in thatrefined French in which our grandfathers not only \
// spoke but thought, andwith the gentle, patronizing intonation natural to a man of importancewho had \
...
//  to her his bald, scented, and shining head,and complacently seated himself on the sofa.";
    ...
    for(int i = 0; i < iteration; i++){
        ...
        smallStringReplace(sstr, insertChar, outputChar);
    ...
    for(int i = 0; i < iteration; i++){
        ...
        auto start = std::chrono::high_resolution_clock::now();
        smallStringReplace(lstr, insertChar, outputChar);
        auto finish = std::chrono::high_resolution_clock::now();
        auto duration = std::chrono::duration_cast<unit>(finish - start).count();
        output+=lstr;
    ...
    for(int i = 0; i < iteration; i++){
        std::string sstr = insertReplacingChars(smallStr, insertChar,  rand()*5+i, 7);
        auto start = std::chrono::high_resolution_clock::now();
        stringStreamReplace(sstr, insertChar, outputChar);
        auto finish = std::chrono::high_resolution_clock::now();
        auto duration = std::chrono::duration_cast<unit>(finish - start).count();
        output+=sstr;
    ...
    for(int i = 0; i < iteration; i++){
        std::string lstr = insertReplacingChars(longStr, insertChar,  rand()*5+i, 7);
        auto start = std::chrono::high_resolution_clock::now();
        stringStreamReplace(lstr, insertChar, outputChar);
        auto finish = std::chrono::high_resolution_clock::now();
        auto duration = std::chrono::duration_cast<unit>(finish - start).count();
        output += lstr;
